        ...
        <div id="match-form">
            ...
            <form action="confirm_action.php" id="myform" method="POST">
                ...
                <select name="lost_item_id" id="lost_item_id" required>
                    <?php
                    $sqlLostItems = "SELECT id, item_name FROM items WHERE item_type = 'lost' AND visibility = 'visible'";
                    $resultLostItems = $conn->query($sqlLostItems);
                    while ($row = $resultLostItems->fetch_assoc()) {
                        echo "<option value='" . safeDisplay($row['id']) . "'>" . safeDisplay($row['id']) . ")" . safeDisplay($row['item_name']) . "</option>";
                    }
                    ?>
                </select><br>
                ...
                <select name="found_item_id" id="found_item_id" required>
                    <?php
                    $sqlFoundItems = "SELECT id, item_name FROM items WHERE item_type = 'found' AND visibility = 'visible'";
                    $resultFoundItems = $conn->query($sqlFoundItems);
                    while ($row = $resultFoundItems->fetch_assoc()) {
                        echo "<option value='" . safeDisplay($row['id']) . "'>" . safeDisplay($row['id']) . ") " . safeDisplay($row['item_name']) . "</option>";
                    }
                    ?>
                </select><br>
            ...
            <div class="item-card-container">
                ...
                if ($result->num_rows > 0) {
                    ...
                    echo "<h4>No pending found items.</h4>";
                }
                ?>
            </div>
        </section>
    ...
    <section id="requested-matched-items">
        ...
        <div class="item-card-container">
            ...
            if ($resultRequested->num_rows > 0) {
                ...
                echo "<h4>No requested matched items.</h4>";
            }
            ?>
        </div>
    </section>
